    ...
    <Card title="今日流量">
      <div className={styles.allStatus}>
        <div className={styles.statusNumWrap}>
          <TrafficItem title="用户总数" today={allUsers} lastDay={allUsers - newUsers[0]} />
          <TrafficItem title="活跃人数" today={activeUsers[0]} lastDay={activeUsers[1]} />
          <TrafficItem title="新用户数" today={newUsers[0]} lastDay={newUsers[1]} />
          <TrafficItem title="访问量" today={pv[0]} lastDay={pv[1]} />

          <TrafficItem title="IP" today={ip[0]} lastDay={ip[1]} />
        </div>
      </div>
    </Card>
  );
