...
export class Monitor {
  ...
  constructor(config: MonitorConfig) {
    ...
    const markUserId = window.localStorage.getItem(
      `web-watch-dog-markUserId-${Monitor.config.appId}`
    );
    if (markUserId) {
    ...
    window.onbeforeunload = () => {
      ...
      img.src = `${api}?data=${encodeURIComponent(JSON.stringify(this.reportStack))}&appId=${
        Monitor.config.appId
      }`;
    };
  }
  ...
  private toReport(data: ReportItem) {
    ...
    (data.appId = Monitor.config.appId), this.reportStack.push(data);
  ...
  private async getWebPerformance() {
    ...
    const getWebvitals = (fn: (data: any) => void): Promise<number> =>
      new Promise(resolve => {
